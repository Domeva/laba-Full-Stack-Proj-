...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if($request['action'] === 'delete'){
        if($id = $sqlMain->deleteEntry($request)){
            echo json_encode(['status' => true, 'entry_id' => $id]);
        }
        die();
    }
    ...
    if(!$sqlMain->checkPhoneNumber($request['phone_number_owner'])){
    ...
    if(!empty($errors)){
        ...
        if($request['action'] === 'add'){
            if($id = $sqlMain->addEntry($request)){
                if($sqlMain->getNewEntry($id)){
                    echo json_encode(['status' => true, 'newEntry' => $sqlMain->getNewEntry($id)]);
                }
            }
        }
        else{
            if($id = $sqlMain->updateEntry($request)){
                if($sqlMain->getNewEntry($id)){
                    echo json_encode(['status' => true, 'newEntry' => $sqlMain->getNewEntry($id)]);
                }
            }
