...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if(empty($password)){
        $errors[] = ['code' => 2, 'message' => 'Password field was empty!'];
    }
    if($username !== '' && !$sqlMain->checkUser($username)){
        $errors[] = ['code' => 3, 'message' => 'Username wasn\'t exists!'];
    }

    if($sqlMain->checkPassword($username, $password)){
        $errors[] = ['code' => 4, 'message' => 'Password is not correct!'];
    }
    ...
    if(!empty($errors)){
        ...
        if($sqlMain->changeUser($username, $password)){

            if($sqlMain->checkCurrentUser($username)){
